...
public class Main {
	public static void main(String args[]) {
		try {
			...
			System.out.println("AStar avec heuristique nulle depuis la racine "+s.getID()+". Chemin obtenu :");
			geograph.printWay(parent, s, t);
			System.out.println("Le graphe de la figure 1 ne faisant pas le lien entre le poids des aretes et la position des sommets, il n'est pas possible de trouver d'heuristique minorante.");
			
			
			
			System.out.println("\n\nQuestion 4 : échiquier");
			Echiquier echiquier = new Echiquier(8, 8, new Point(4,1), new Point(4, 7));
			echiquier.createEdgesForApawn();
			ends = new ArrayList<Vertex<Point>>();
			ends.add(echiquier.getDestination());
			Heuristique h = new Heuristique<Long>(echiquier.getNbVertices());
			...
			System.out.println("A* d'un pion dans un échiquier de 8 sur 8 :\n"+echiquier);
			System.out.println("Nous avons choisi de représenter le déplacement d'un pion d'un jeu d'echec sur un echiquier standard.\nAinsi le premier deplacement peut etre de 2 cases.");
			System.out.println("De plus le point d'arrivee est la derniere case accessible.");	
			System.out.println("L'heuristique choisie est la distance euclidienne entre les sommets.");	
